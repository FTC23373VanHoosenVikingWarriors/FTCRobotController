...
public class _VWAutoIntoDeep  extends LinearOpMode {
    ...
    static final int    HBASKET_POS_ARM_ENCODE_VALUE    =   1725;     //
...
Telemetry: This continuously updates the arm and robot positions while the OpMode is active.
        ...
        TrajectorySequence sequence = drive.trajectorySequenceBuilder(new Pose2d())
                .forward(50) // Move forward 50 inches
                .addTemporalMarker(0.5, () -> {
                    // Start extending the arm when at 30 inches
                ...
                .waitSeconds(0.5) // Optional: Wait a moment for arm to extend
                .addTemporalMarker(1.0, this::GripperOpen)
                .back(40) // Move back 40 inches
                .addTemporalMarker(0.5, () -> {
                    // Retract the arm after backing up 30 inches
                    if (drive.getPoseEstimate().getY() <= 10) { // Check if moved back 30 inches
                        GripperClose();
                        MoveArm(0);
